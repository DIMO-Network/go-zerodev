...
// IsFleetInstalled returns true when (a) the kernel is deployed AND (b) the
// weighted-ECDSA validator is installed with selector permission for
// executeUserOp. If either is false, the caller must send an enable-mode
// UserOp (SendInstallAndCall) for its next action on this kernel rather
// than a plain default-mode one.
//
// The deployment check is folded in because isAllowedSelector reverts on a
