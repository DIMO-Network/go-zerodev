...
//
// The full EntryPoint nonce is then (key << 64) | seq, where seq comes
// from EntryPoint.getNonce(account, key).
...
// NonceKeyDefaultModeForWeightedEcdsa is shorthand for the regular-mode
// nonce key targeting the weighted-ECDSA secondary validator. Every fleet
// UserOp uses this shape.
...
// FullNonce composes the EntryPoint's 256-bit nonce from a 24-byte key
// and 8-byte sequence (as returned by EntryPoint.getNonce).
